...
QR delivery does not depend on model output. Chat text is composed by the LLM, but an omitted attachment marker silently means the operator receives no QR code, which was observed in practice. When `[recebi.qr_delivery]` is configured, `recebi_create_request` and `recebi_render_qr` schedule a delayed send through the trusted local host command and report `qr_delivery`. The call passes fixed arguments with no shell, sends only a locally derived marker as the message body, is bounded by a 20-second timeout, and is fail-open: a delivery failure never invalidates the created receivable or the rendered artifact. The send is delayed by a bounded interval, six seconds by default, because a tool call returns in milliseconds while the model needs seconds to reply; without it the image would arrive before the message explaining it. That is best-effort ordering, not a guarantee, and a scheduled send is drained before the process exits. It grants no payment, signing, or configuration authority. Without that configuration, behavior is unchanged and the marker is returned for the model to emit.
